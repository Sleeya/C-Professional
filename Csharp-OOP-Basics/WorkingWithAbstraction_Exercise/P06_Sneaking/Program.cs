...
class Sneaking
{
    ...
    private static void MoveFigures(char[] moves)
    {
        for (int i = 0; i < moves.Length; i++)
        {
            ...
            FindEnemyOnSamsRow(getEnemy);
            TryToKillSam(getEnemy);
            MoveSam(moves, i, getEnemy);
        }
    }

    private static void MoveSam(char[] moves, int i, int[] getEnemy)
    {
        ...
        TryToKillNikoladze(getEnemy);
    }

    private static void TryToKillNikoladze(int[] getEnemy)
    {
    ...
    private static void TryToKillSam(int[] getEnemy)
    {
    ...
    private static void FindEnemyOnSamsRow(int[] getEnemy)
    {
        for (int j = 0; j < room[samPosition[0]].Length; j++)
        {
            if (room[samPosition[0]][j] != '.' && room[samPosition[0]][j] != 'S')
    ...
    private static void MoveOrTurnEnemys()
    {
        for (int row = 0; row < room.Length; row++)
        {
            for (int col = 0; col < room[row].Length; col++)
    ...
    private static void InitializePlayField(int n)
    {
        room = new char[n][];
